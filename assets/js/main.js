...
function fmtUGX(n){ return 'UGX ' + Number(n).toLocaleString('en-US'); }

var Cart = {
  read: function(){
    try { return JSON.parse(localStorage.getItem('lb-cart')) || {}; }
    catch (e) { return {}; }
  },
  write: function(c){ localStorage.setItem('lb-cart', JSON.stringify(c)); updateCartBadge(); },
  add: function(id, qty){ var c = this.read(); c[id] = (c[id] || 0) + (qty || 1); this.write(c); },
  setQty: function(id, qty){ var c = this.read(); if (qty <= 0) { delete c[id]; } else { c[id] = qty; } this.write(c); },
  remove: function(id){ this.setQty(id, 0); },
  clear: function(){ localStorage.removeItem('lb-cart'); updateCartBadge(); },
  count: function(){
    var c = this.read(), n = 0;
  ...
  items: function(){
    ...
    return Object.keys(c)
      .map(function(id){ return { product: findProduct(id), qty: c[id] }; })
      .filter(function(i){ return i.product; });
  },
  subtotal: function(){
    return this.items().reduce(function(s, i){ return s + i.product.price * i.qty; }, 0);
  }
};
...
function initAgeGate(){
  ...
  gate.innerHTML =
    '<div class="max-w-sm w-full text-center p-10" style="background:var(--parchment);">' +
      '<div class="mono mx-auto mb-5">LB</div>' +
